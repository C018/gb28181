...
4. **快照自动清理** - 设备删除时自动清理相关通道快照
5. **录像功能** - 录像回放和控制 API
6. **账号密码登录** - 通过配置文件管理账号密码
7. **网页修改账号密码** - API 支持修改用户凭据
8. **所有接口鉴权访问** - JWT 令牌认证保护所有敏感端点
9. **GB28181 设备 ID 验证** - 20 位标准长度校验
10. **MySQL 数据库支持** - 支持 SQLite/PostgreSQL/MySQL
11. **反向代理** - 流媒体服务反向代理
...
**参考文档**: `docs/FRONTEND_INTEGRATION.md` 提供完整的前端集成指南

### 📋 待实现的功能

1. **ONVIF 完整支持** - 需要扩展 ONVIF 协议功能
2. **国际化 (i18n)** - 支持中文和 English
3. **PTZ 预置位查询** - GB28181 标准限制，需要本地存储或厂商扩展
...
### API 端点
```
GET  /channels/{id}/snapshot         # 获取快照
POST /channels/{id}/refresh-snapshot # 刷新快照
```
...
### 预置位查询限制
**GB28181 标准说明**:
- GB28181 协议标准不包含预置位查询命令
- 预置位信息存储在设备端，无标准接口返回预置位列表
- 当前 `/ptz/presets` 端点返回空列表

**解决方案**:
1. 在本地数据库存储预置位设置记录（推荐）
2. 使用设备厂商的私有扩展协议
3. 手动维护预置位编号列表
